...
def test_primary_rate_limit_uses_fallback_without_exposing_secret():
    assert importlib.util.find_spec('jev_lcm_hermes_compaction.providers'), 'provider chain missing'
    ...
    chain = ProviderChain(Settings(), {'TYPESAFE_API_KEY': 'private-one', 'OPENROUTER_API_KEY': 'private-two'}, transport)
    assert chain.score({}, {'x': {'type': 'noul', 'instructions': 'keep?'}})['x'] == 0.19
    assert len(seen) == 2
