...
def test_batched_anchors_are_exact_and_oversized_batch_is_explicit():
    assert importlib.util.find_spec('jev_lcm_hermes_compaction.prepass'), 'prepass missing'
    ...
    messages = [{'role': 'user', 'content': 'protected'}, {'role': 'assistant', 'content': 'The delegation id is `abc123def456`. We must retain it.'}]
    for i in range(541):
        messages += [{'role': 'assistant', 'tool_calls': [{'id': str(i), 'type': 'function', 'function': {'name': 'read', 'arguments': '{}'}}]}, {'role': 'tool', 'tool_call_id': str(i), 'content': 'x' * 100}]
