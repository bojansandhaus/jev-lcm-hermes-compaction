...
def test_real_compaction_and_recall_with_failed_jev(tmp_path,monkeypatch):
    engine=JevLCMContextCompressor(config=LCMConfig(database_path=str(tmp_path/'lcm.db'),fresh_tail_count=2,fresh_tail_max_tokens=500,leaf_chunk_tokens=500),hermes_home=str(tmp_path))
    engine.on_session_start('integration')
    messages=[{'role':'user','content':'first'}]
    for i in range(12): messages.extend([{'role':'user','content':('User facts %s. '%i)*100},{'role':'assistant','content':('Assistant facts %s. '%i)*100+' identifier `abc123def456`.'}])
...
def test_541_unscored_raw_recovery(tmp_path):
    ...
    engine=JevLCMContextCompressor(config=LCMConfig(database_path=str(tmp_path/'lcm.db'),fresh_tail_count=2,fresh_tail_max_tokens=500,leaf_chunk_tokens=500),hermes_home=str(tmp_path),settings=settings)
    engine.on_session_start('bulk')
    messages=[{'role':'user','content':'first'}]
    for i in range(541):messages.extend([{'role':'assistant','tool_calls':[{'id':str(i),'type':'function','function':{'name':'read','arguments':'{}'}}]}, {'role':'tool','tool_call_id':str(i),'content':'evidence%s '%i+'x'*100}])
    messages.extend([{'role':'user','content':'fresh'},{'role':'assistant','content':'fresh answer'}])
    engine.jev=Prepass(settings,ProviderChain(settings,{'OPENROUTER_API_KEY':'synthetic'},lambda u,k,p,t:{'answers':{q:{'noul':.19} for q in p['questions']}}))
