...
class Settings:
    ...
    jev_fallback_on: tuple[str, ...] = ("transport_error", "timeout", "401", "403", "429", "5xx")
    ...
    jev_anchor_patterns: tuple[str, ...] = (
        r"\b[a-f0-9]{7,40}\b", r"\b[A-Z_]{2,}_(?:KEY|TOKEN|SECRET|URL|PATH|ID)\b",
        r"[^.!?\n]*(?:root cause|because|constraint|must|never|always)[^.!?\n]*[.!?]?",
        r"(?:\bline \d+\b|:[0-9]+:[0-9]+)", r"`[^`\n]+`",
        r"[\"'][^\"'\n]*(?:/|\\)[^\"'\n]*[\"']", r"\b[vV]?\d+\.\d+\.\d+(?:[+.-][\w.]+)?\b",
    )
    ...
    def __post_init__(self) -> None:
        ...
        if not self.jev_fallback_order or len(set(self.jev_fallback_order)) != len(self.jev_fallback_order) or any(p not in ("typesafe", "openrouter") for p in self.jev_fallback_order):
            raise ValueError("invalid jev_fallback_order")
        for value in (self.keep_threshold, self.keep_threshold_max, self.min_keep_rate, self.jev_urgent_context_ratio):
            if not 0 <= value <= 1:
                raise ValueError("probability outside [0, 1]")
        for value in (self.jev_batch_window_turns, self.jev_max_candidates_per_batch, self.max_state_tokens, self.max_request_tokens, self.hint_budget_tokens):
        ...
        if self.request_timeout_s <= 0 or self.jev_fallback_cooldown_s < 0 or self.jev_fallback_max_retries < 0 or self.min_result_chars < 0 or self.truncate_head_chars < 0:
...
def endpoint(base: str, path: str) -> str:
    ...
    if "\\" in decoded or any(ord(c) < 33 for c in decoded) or parsed.scheme not in ("https", "http") or not parsed.netloc or parsed.username or parsed.password or parsed.query or parsed.fragment:
