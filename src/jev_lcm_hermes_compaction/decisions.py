...
def questions(candidate: Candidate) -> dict[str, Any]:
    ...
    return {candidate.id + ":" + name: {"type": "noul", "instructions": instruction + " Candidate id: " + candidate.id} for name, instruction in instructions.items()}


def decide(candidate: Candidate, scores: dict[str, float], threshold: float, floor_keep: bool = False) -> None:
    names = ("anchor_keep", "recovery") if candidate.kind == "anchor" else ("keep_call", "keep_result")
    candidate.scores = {name: scores[candidate.id + ":" + name] for name in names}
    candidate.jev_unscored = False
    if candidate.kind == "anchor":
        candidate.action = "keep" if candidate.scores["anchor_keep"] >= threshold or floor_keep else "drop"
