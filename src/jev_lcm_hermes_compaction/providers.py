...
class JevProvider:
    ...
    def score(self, state: Any, questions: dict[str, Any], key: str, timeout: float, transport: Transport) -> dict[str, float]:
        payload = {"model": self.model, "state": state, "questions": questions}
        return parse_answers(transport(self.url, key, payload, timeout), list(questions))
...
class ProviderChain:
    def __init__(self, settings: Settings, env: Mapping[str, str] | None = None, transport: Transport = post, clock: Callable[[], float] = time.monotonic):
        self.settings, self.transport, self.clock = settings, transport, clock
        self._keys = {name: (env if env is not None else os.environ).get(var, "").strip() for name, var in ENV.items()}
        configured = settings.jev_fallback_order if settings.jev_provider == "auto" else (settings.jev_provider,)
        ...
        self.providers: dict[str, JevProvider] = {"typesafe": TypeSafeProvider(settings), "openrouter": OpenRouterProvider(settings)}
    ...
    def diagnostics(self) -> dict[str, Any]:
        return {"order": self.order, "keys_present": [ENV[p] for p in ENV if self._keys[p]], "cooldown_seconds": {p: max(0.0, t - self.clock()) for p, t in self.cooldowns.items()}, "last_errors": self.errors, "last_provider": self.last_provider}
    ...
    def score(self, state: Any, questions: dict[str, Any]) -> dict[str, float]:
        ...
        for index, name in enumerate(available):
            if name != self.order[0]:
                ...
                LOG.warning("jev_provider_fallback from=%s to=%s reason=%s", previous, name, self.errors.get(previous, "cooldown"))
            attempts = 1 if index + 1 < len(available) else 1 + self.settings.jev_fallback_max_retries
            for attempt in range(attempts):
                self.calls += 1
                try:
                    scores = self.providers[name].score(state, questions, self._keys[name], self.settings.request_timeout_s, self.transport)
