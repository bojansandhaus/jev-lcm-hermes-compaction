...
class JevThresholdCalibrator:
    def __init__(self, window: int = 500, minimum: int = 50,
                 fallback: float = 0.15, cap: float = 0.40,
                 keep_rate: float = 0.10, enabled: bool = True,
        ...
        self.keep_rate, self.enabled, self.conservative = keep_rate, enabled, conservative
        self.current, self.calibrated = fallback, False

    def observe(self, values: Iterable[float]) -> float:
        samples = list(values)
        if not all(isinstance(v, (int, float)) and not isinstance(v, bool)
                   and 0 <= v <= 1 for v in samples):
        ...
        if self.calibrated:
            ...
            if q < 0:
                ...
                self.current = min(self.cap, data[lo] + (data[hi] - data[lo]) * (position - lo))
        return self.current
