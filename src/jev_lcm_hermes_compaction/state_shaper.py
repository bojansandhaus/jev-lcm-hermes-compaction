...
def shape(
    messages: list[dict[str, Any]], candidates: list[Candidate], settings: Settings
) -> tuple[dict[str, Any], dict[str, Any], list[Candidate], int]:
    state: dict[str, Any] = {}
    tier = 0
    for tier in range(5):
        ...
        for message in history:
            ...
            if tier and message.get("role") == "tool":
                ...
                message["content"] = (
                    str(text)[:head] + " [result omitted; raw evidence retained]"
                )
            elif tier >= 2:
                message["content"] = " ".join(str(text).split())[
                    : 200 if tier == 2 else 60
                ]
            for call in message.get("tool_calls", []):
                fn = call.get("function", {})
                if tier:
                    fn["arguments"] = str(fn.get("arguments", ""))[
                        : (1000, 200, 60, 30)[tier - 1]
                    ]
    ...
    for candidate in candidates[: settings.jev_max_candidates_per_batch]:
        ...
        if (
            tokens(trial) > settings.max_state_tokens
            or tokens(envelope) > settings.max_request_tokens
        ):
            continue
