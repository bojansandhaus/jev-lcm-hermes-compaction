...
class JevLCMContextCompressor(LCMEngine):
    ...
    def _ingest_messages(self, messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        try:
            ...
            self.jev.collect(working, tail_start, self._get_store_id_map_for_messages(working))
            ...
            logging.getLogger(__name__).warning('Jev prepass failed; LCM default condensation')
        return working
    ...
    def _summarize_leaf_chunk_with_rescue(self, initial_chunk: Any, focus_topic: str | None = None, deadline: float | None = None) -> Any:
        hint = self.jev.hint_block()
    ...
    def compress(self, messages: list[dict[str, Any]], current_tokens: int = 0, focus_topic: str | None = None, force: bool = False) -> list[dict[str, Any]]:
        ...
        try:
            result = super().compress(messages, current_tokens=current_tokens, focus_topic=focus_topic, force=force)
        finally:
            self._jev_compacting = False
    ...
    def handle_tool_call(self, name: str, args: dict[str, Any], **kwargs: Any) -> str:
        ...
        if name in ('jev_scores', 'jev_anchors'):
            return json.dumps([{'id': c.id, 'store_id': c.store_id, 'kind': c.kind, 'scores': c.scores, 'action': c.action, 'jev_unscored': c.jev_unscored} for c in self.jev.candidates.values() if name != 'jev_anchors' or c.kind == 'anchor'])
