...
The independent review identified overlapping DSH flush calls and a potential concurrent Hermes ingestion race. DSH scoring is now serialized per Prepass. The Hermes adapter uses a reentrant per-engine lock around ingestion, compaction, tool handling, and turn/end/reset callbacks. Clones receive independent locks; the vendored upstream engine is unchanged.
